(async () => {
  ...
  drawPolygon(shapes, 250, 510, 4, 80, 80, FMath.QUARTER_PI);
  ...
  drawPolygon(shapes, 550, 510, 6, 60, 60, FMath.HALF_PI);
  ...
  function drawPolygon(graphics, x, y, sides, radius1, radius2, angle) {
    ...
    const startAngle = angle - FMath.HALF_PI;

    // get all points
    const slice = FMath.TAU / sides;
    const polygon = [];
